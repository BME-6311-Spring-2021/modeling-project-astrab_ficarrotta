...
fibroblasts-own [tgf-level active-status]

to setup
  __clear-all-and-reset-ticks           ; clear all
  set  average-fibrosis-level 0
  ...
  ask patch 0 0 [ask patches in-radius 15[
     sprout-stiffs 1[
      set shape "square"
      set color red
      set size 0.7
...
to add-fibroblasts
  ask patch 0 0 [
    ask n-of number-of-fibroblasts patches in-radius 15[
      sprout 1        ; create the number of initial bacteria as designated by the slider on the interface tab
      [
        set breed fibroblasts            ; denote the 'breed' of the turtle as an "bacteria" cell
        set shape "airplane"            ; denote the shape of the turtle as an bacteria cell shape
        set color pink + 2
...
to secrete-tgf
  ask patches with [count macrophages-here > 0] [
    let num-macrophages count macrophages-here
    set tgf tgf + macrophage-tgf-release-rate * (num-macrophages)
  ]
end

to secrete-tgf2
  ask patches with [count neutrophils-here > 0] [
    let num-neutrophils count neutrophils-here
    set tgf tgf + neutrophil-tgf-release-rate * (num-neutrophils)
  ]
end

to consume-tgf
  ask patches with [count fibroblasts-here > 0] [
    let num-fibroblasts count fibroblasts-here
    ask fibroblasts-here [set tgf-level tgf-level - 1]
    if tgf > 30[
      ask fibroblasts-here [set tgf-level tgf-level + 2]
      set tgf tgf - 30 * num-fibroblasts
  ]]
...
to migrate
  ...
  ask fibroblasts
  [
     ifelse any? macrophages in-radius fibroblast-homing-range [
      face min-one-of macrophages [distance myself]
      fd 1
    ]
    [move-to one-of neighbors]
  ]
...
to activate-fibroblasts
  ask fibroblasts with [tgf-level > active-threshold][
...
to fibrose
  ask patches with [count fibroblasts-here with [active-status = true] > 0] [
    let num-fibroblasts count fibroblasts-here with [active-status = true]
    ask stiffs-here[
    set fibrosis fibrosis + collagen-deposition-rate * (num-fibroblasts)
    set color scale-color red fibrosis 15 -15
  ]]
end

to calculate-average-fibrosis
  set average-fibrosis-level mean [fibrosis] of stiffs
end
@#$#@#$#@
GRAPHICS-WINDOW
623
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot average-fibrosis-level"
...
infect</setup>
  ...
  <experiment name="experiment" repetitions="30" runMetricsEveryStep="true">
    <setup>setup
infect
add-fibroblasts</setup>
    <go>Run-Simulation</go>
    <exitCondition>ticks = 504</exitCondition>
    <metric>average-fibrosis-level</metric>
    <enumeratedValueSet variable="macrophage-tgf-release-rate">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="active-threshold">
      <value value="10"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="fibroblast-homing-range">
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Macrophage_response">
      <value value="&quot;Low&quot;"/>
    ...
    <enumeratedValueSet variable="neutrophil-tgf-release-rate">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="collagen-deposition-rate">
      <value value="0.6"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Prolif-rate">
      <value value="2"/>
    ...
    <enumeratedValueSet variable="number-of-fibroblasts">
      <value value="50"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
